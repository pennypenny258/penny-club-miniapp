...
const {resolveAdminGovernanceConfig}=require('../server/src/auth/admin-governance-config');
const issues = [];
...
if (process.env.NODE_ENV === 'production') {
  ...
  try{const governance=resolveAdminGovernanceConfig(process.env);if(!governance.enabled)issues.push('首次管理员引导、双人角色治理与脱敏审计仍保持禁用')}catch(error){issues.push(`生产后台治理未就绪：${error.message}`)}
