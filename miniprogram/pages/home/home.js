...
const labels={member:'会员',resource:'资料',activity:'活动',opportunity:'机会',meeting_replay:'回放',industry_report:'研究报告',group_digest:'群聊精华'};
Page({
  data:{items:[],favorites:[],query:''},
  onShow(){const query=getApp().globalData.publicSearchQuery||'';this.setData({query});this.loadFeed()},
  onSearch(e){const query=e.detail.value;getApp().globalData.publicSearchQuery=query;this.setData({query});clearTimeout(this.timer);this.timer=setTimeout(()=>this.loadFeed(),300)},
