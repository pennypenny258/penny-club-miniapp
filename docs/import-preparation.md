...
预检按统一标准金额生成身份候选：200 元天使轮股东、666 元 A1 轮股东、799 元 A2 轮股东、499 元荣誉董事。未命中价格进入人工归类，任何候选都不能自动激活会籍。商户号小票与手动转账分别使用自己的姓名、发生时间、手机号和金额四类最简字段，不受微信小店字段约束。
...
付款预检只返回识别到的安全字段类别、读取行数、可匹配/待人工核对、价格待归类、四档身份候选和可能退款等汇总计数；不会返回逐行订单、完整原始表头、姓名、手机号、价格、订单号、地址或备注正文。
