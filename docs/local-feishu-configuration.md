...
当前代码已准备 CloudBase PG Storage 的服务端适配器和 005 迁移包，但尚未接入真实业务路由，也未创建 Bucket 或配置凭据；只填写 `PRIVATE_STORAGE_PROVIDER` 不会自动启用。完整边界和未来顺序见 `cloudbase-private-resource-storage.md`。

本项目已选择 CloudBase PG 的 HTTPS Storage API 路线，不需要标准版私有网络或数据库外网 IPv4；真实启用仍必须先完成身份、迁移、私有 Bucket、备份恢复和集成验收。
