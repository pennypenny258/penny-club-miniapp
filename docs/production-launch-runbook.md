...
GitHub `main` 提交 `92c5448` 已构建为 `penny-club-prod-api-012`，CloudBase 显示服务状态 `normal` 且 `012` 承载 100% 流量。`api.pennysclub.com` 的 CNAME、TLS、生产健康、正式 RBAC 登录、合成 CSV 持久化/回滚、合成活动和清理均已通过；临时管理员和合成业务数据无残留。CloudBase 标准版于 2026-09-10 23:59:59 到期，控制台仍显示资源临时隔离；套餐恢复并重跑全部门禁前，不得上传真实数据。
...
- 新版本已经完成可回滚的版本切换并承载 100%，旧版本不再承载业务流量。

## 本次发布结果与套餐恢复后的复验顺序

1. 已确认 GitHub `main` 为 `92c5448`，包含 CSV 文本上传修复、生产页面/API 安全响应头和小程序 production 默认档位。
2. 已构建 `012`，并使用非敏感查询标记 `pennys_canary=prod-intake-final`，在可回滚的版本切换后完成健康、readiness、静态资源哈希、正式登录、RBAC、CSV 和活动合成验收。
3. 已把 `012` 切换到 100%，确认旧演示接口 404、受保护接口匿名 401、生产入口无内存回退。
4. 已回滚并删除合成业务行、撤销临时会话/角色/绑定、删除 CloudBase 临时用户；脱敏审计保留。
5. 账号所有者恢复 CloudBase 套餐后，先确认隔离提示消失，再只读核对数据库、手工备份、`012` 的 100% 流量、正式域名和主管理员绑定。
6. 套餐恢复后的所有检查通过后，才解除“禁止真实数据”门禁。
7. 把 `https://api.pennysclub.com` 加入微信公众平台 `request` 合法域名，并完成开发者工具与真机检查。

套餐隔离期间，URL 参数灰度曾出现控制面记录与实际路由不一致，已立即回退，未把该结果用于放行。后续如再次使用参数灰度，必须同时验证“匹配参数进入新版本”和“不匹配参数留在旧版本”，任一不成立即回退。
...
- 手工恢复点 `87679197874494` 已创建并显示成功，大小 2.10 GB、进度 100%、保留至 2099-12-31；自动全量备份 `87652892369237` 于 2026-09-10 成功，自动全量与日志备份保留 7 天。
- `pnpm db:restore-rehearsal` 会在两个临时、无端口、tmpfs PostgreSQL 容器中用纯合成哨兵执行 `pg_dump → pg_restore`，核对迁移清单、数据摘要、私有权限、公开视图无密文字段和审计不可修改，并输出本地 RPO/RTO 证据；脚本不接受数据库 URL，不能接触生产或真实资料。2026-09-11 实跑通过：`001–008 + 014` 共 9 个生产迁移、39 张私有表、19 个相关函数及用户/活动/审计哨兵恢复前后摘要一致，恢复阶段少于 1 秒，审计更新被拒绝。
- 本地演练只能证明应用级恢复步骤。套餐恢复后仍必须用手工恢复点做一次隔离的云快照克隆恢复并运行同等只读检查，随后销毁克隆；这一步可能产生云资源费用，必须由账号所有者授权后执行。未完成云快照克隆恢复前，备份状态记为“恢复点可用、云恢复待验收”，不能把“备份成功”等同于“已证明可恢复”。
- CloudBase 托管数据库当前不能直接启用删除保护，因此删除环境或实例前必须再次核对恢复点。
