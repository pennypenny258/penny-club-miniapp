...
| 实体 | 用途 | 关键字段 |
|---|---|---|
| users / external_identity_bindings | 微信身份 | 服务端 code2Session 得到的 app-scoped openid 仅生成带密钥哈希；昵称、微信号、群昵称由用户填写或运营导入，绝不假设登录接口返回；手机号唯一匹配且 CRM 有效时可按服务端规则自动绑定，其余人工复核 |
...
- 小程序登录不提供个人微信群成员状态；`group_status` 只能来自 CRM 运营维护。手机号仅在用户主动触发手机号授权后经服务端验证。只有手机号/OpenID 对应唯一 CRM、账号正常、当前在群、月份有效、无缺项/冲突/风险且 004 投影可放行时自动绑定；微信号和群昵称只是辅助线索，其他情况全部人工复核。
