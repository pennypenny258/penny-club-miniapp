...
| 实体 | 用途 | 关键字段 |
|---|---|---|
| users / external_identity_bindings | 微信身份 | 服务端 code2Session 得到的 app-scoped openid 仅生成带密钥哈希；昵称、微信号、群昵称由用户填写或运营导入后人工确认，绝不假设登录接口返回 |
...
- 小程序登录不提供个人微信群成员状态；`group_status` 只能来自 CRM 运营确认。手机号仅在用户主动触发手机号授权后经服务端验证，微信号和群昵称仍是用户/运营提供的匹配线索，均不能自动绑定或放行会籍。
