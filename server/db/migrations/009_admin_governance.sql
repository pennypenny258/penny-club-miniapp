-- CloudBase ExecutePGSql-compatible, forward-only admin governance migration.
-- One top-level DO statement: the platform receives one SQL statement and PostgreSQL applies it atomically.
-- Depends on 008. Creates no identity binding, role assignment or bootstrap authorization row.
DO $admin_governance_migration$
BEGIN
  EXECUTE $ddl$
    CREATE TABLE IF NOT EXISTS venture_private.admin_bootstrap_authorizations (
      ...
      subject_hash char(64) NOT NULL UNIQUE CHECK(subject_hash ~ '^[0-9a-f]{64}$'),
      ticket_hash char(64) NOT NULL UNIQUE CHECK(ticket_hash ~ '^[0-9a-f]{64}$'),
      status text NOT NULL DEFAULT 'ready' CHECK(status IN ('ready','used','expired','revoked')),
      expires_at timestamptz NOT NULL,
      control_reference text NOT NULL CHECK(control_reference ~ '^[A-Za-z0-9._:-]{8,128}$'),
  ...
  EXECUTE $ddl$
    CREATE TABLE IF NOT EXISTS venture_private.admin_role_change_requests (
      id text PRIMARY KEY,
      target_user_id text NOT NULL REFERENCES venture_private.users(id),
      role_id text NOT NULL REFERENCES venture_private.admin_roles(id),
      action text NOT NULL CHECK(action IN ('grant','revoke')),
      requester_id text NOT NULL REFERENCES venture_private.users(id),
      approver_id text REFERENCES venture_private.users(id),
      status text NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','applied','rejected','cancelled')),
      idempotency_reference text NOT NULL UNIQUE CHECK(idempotency_reference ~ '^[A-Za-z0-9._:-]{3,127}$'),
      requested_at timestamptz NOT NULL DEFAULT now(),
      approved_at timestamptz,
      applied_assignment_id text,
      CHECK(requester_id<>target_user_id),
      CHECK(approver_id IS NULL OR (approver_id<>requester_id AND approver_id<>target_user_id))
    )
  $ddl$;

  EXECUTE 'ALTER TABLE venture_private.admin_bootstrap_authorizations ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE venture_private.admin_bootstrap_authorizations FORCE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE venture_private.admin_role_change_requests ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE venture_private.admin_role_change_requests FORCE ROW LEVEL SECURITY';
  EXECUTE 'REVOKE ALL ON ALL TABLES IN SCHEMA venture_private FROM PUBLIC';
  EXECUTE 'REVOKE ALL ON ALL TABLES IN SCHEMA venture_private FROM anon';
  EXECUTE 'REVOKE ALL ON ALL TABLES IN SCHEMA venture_private FROM authenticated';

  EXECUTE $ddl$
    CREATE OR REPLACE FUNCTION venture_private.require_fresh_system_admin(p_session_id text,p_actor_id text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER SET search_path=venture_private,pg_catalog AS $function$
    DECLARE session record;
    BEGIN
      SELECT * INTO session FROM venture_private.admin_sessions WHERE id=p_session_id FOR UPDATE;
      IF NOT FOUND OR session.user_id<>p_actor_id OR session.status<>'active' OR session.expires_at<=now()
        OR session.step_up_verified_at IS NULL OR session.step_up_verified_at<now()-interval '15 minutes'
        OR NOT venture_private.admin_has_permission(p_actor_id,'admin.role.manage')
      THEN RAISE EXCEPTION 'fresh system admin authorization required'; END IF;
    END
    $function$
  $ddl$;
  ...
  EXECUTE $ddl$
    ...
    LANGUAGE plpgsql SECURITY DEFINER SET search_path=venture_private,pg_catalog AS $function$
    DECLARE authorization record; role_id text; binding_id text; assignment_id text;
    BEGIN
      ...
      SELECT id INTO role_id FROM venture_private.admin_roles WHERE code='system_admin' AND status='active';
      IF role_id IS NULL THEN RAISE EXCEPTION 'system admin role unavailable'; END IF;
      binding_id:='admin-binding-'||md5(random()::text||clock_timestamp()::text);
      assignment_id:='admin-assignment-'||md5(random()::text||clock_timestamp()::text);
      ...
      RETURN jsonb_build_object('status','bootstrapped','role_code','system_admin','self_registration',false);
    END
    $function$
  $ddl$;
  ...
  EXECUTE $ddl$
    ...
    LANGUAGE plpgsql SECURITY DEFINER SET search_path=venture_private,pg_catalog AS $function$
    DECLARE role_id text; request_id text; existing record;
    BEGIN
      PERFORM venture_private.assert_cloudbase_service_role();
      PERFORM venture_private.require_fresh_system_admin(p_requester_session_id,p_requester_id);
      ...
      SELECT id INTO role_id FROM venture_private.admin_roles WHERE code=p_role_code AND status='active';
      IF role_id IS NULL THEN RAISE EXCEPTION 'role unavailable'; END IF;
      SELECT * INTO existing FROM venture_private.admin_role_change_requests WHERE idempotency_reference=p_idempotency_reference;
      IF FOUND THEN
        IF existing.requester_id<>p_requester_id OR existing.target_user_id<>p_target_user_id OR existing.role_id<>role_id OR existing.action<>p_action THEN RAISE EXCEPTION 'role change idempotency conflict'; END IF;
        RETURN jsonb_build_object('request_id',existing.id,'status',existing.status,'reused',true);
      END IF;
      request_id:='role-change-'||md5(random()::text||clock_timestamp()::text);
      INSERT INTO venture_private.admin_role_change_requests(id,target_user_id,role_id,action,requester_id,idempotency_reference) VALUES(request_id,p_target_user_id,role_id,p_action,p_requester_id,p_idempotency_reference);
      INSERT INTO venture_private.audit_logs(id,actor_user_id,actor_role,action,subject_type,subject_id,safe_change_summary) VALUES('audit-'||md5(random()::text||clock_timestamp()::text),p_requester_id,'system_admin','admin.role_change.request','admin_role_change',request_id,jsonb_build_object('role_code',p_role_code,'change_action',p_action,'sensitive_data_included',false));
  ...
  EXECUTE $ddl$
    ...
    LANGUAGE plpgsql SECURITY DEFINER SET search_path=venture_private,pg_catalog AS $function$
    DECLARE request record; assignment_id text;
    BEGIN
      PERFORM venture_private.assert_cloudbase_service_role();
      PERFORM venture_private.require_fresh_system_admin(p_approver_session_id,p_approver_id);
      SELECT role_change.*,role.code AS role_code INTO request FROM venture_private.admin_role_change_requests role_change JOIN venture_private.admin_roles role ON role.id=role_change.role_id WHERE role_change.id=p_request_id FOR UPDATE OF role_change;
      ...
      IF request.action='grant' THEN
        ...
        assignment_id:='admin-assignment-'||md5(random()::text||clock_timestamp()::text);
        INSERT INTO venture_private.admin_role_assignments(id,user_id,role_id,granted_by) VALUES(assignment_id,request.target_user_id,request.role_id,p_approver_id);
      ELSE
        UPDATE venture_private.admin_role_assignments SET revoked_at=now() WHERE user_id=request.target_user_id AND role_id=request.role_id AND revoked_at IS NULL;
      ...
      RETURN jsonb_build_object('request_id',p_request_id,'status','applied','sessions_revoked',request.action='revoke');
    END
    $function$
  $ddl$;
  ...
  EXECUTE $ddl$
    ...
    LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=venture_private,pg_catalog AS $function$
    DECLARE session record;
    BEGIN
      PERFORM venture_private.assert_cloudbase_service_role();
      SELECT * INTO session FROM venture_private.admin_sessions WHERE id=p_session_id;
