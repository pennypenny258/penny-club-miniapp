...
class FormalFirstBindingService{
  ...
  async start({code,phoneGrantCode,wechatId,groupNickname}={}){
    const exchanged=await this.exchangeClient.exchange(code),{subjectHash}=hashWechatSubject({config:this.identityConfig,...exchanged}),matchTokens={},evidence=[];
    ...
    const result=await this.repository.stageCandidate({subjectHash,matchTokens,evidence:{evidenceTypes:evidence,phoneEvidence:candidate.phoneEvidence,wechatIdEvidence:candidate.wechatIdEvidence,groupNicknameEvidence:candidate.groupNicknameEvidence,automaticMembershipActivation:false}});
