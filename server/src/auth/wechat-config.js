...
function resolveWechatIdentityConfig(environment=process.env){
  ...
  const anyIdentityConfig=['WECHAT_MINIPROGRAM_APP_ID',...sensitiveKeys,'MEMBER_SESSION_ISSUER','MEMBER_SESSION_AUDIENCE','MEMBER_SESSION_REVOCATION_STORE','MEMBER_BINDING_MODE'].some(key=>present(environment[key]));
  ...
  if(environment.MEMBER_BINDING_MODE!=='crm_exact_match_or_operator_review')throw new Error('MEMBER_BINDING_MODE 必须为 crm_exact_match_or_operator_review；仅唯一且有效的 CRM 匹配可自动绑定');
  ...
  const apiTimeoutMs=integer(environment.WECHAT_API_TIMEOUT_MS,5000,1000,15000,'WECHAT_API_TIMEOUT_MS');
  const apiMaxResponseBytes=integer(environment.WECHAT_API_MAX_RESPONSE_BYTES,16384,1024,65536,'WECHAT_API_MAX_RESPONSE_BYTES');
  const accessTokenRefreshSkewSeconds=integer(environment.WECHAT_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,120,30,600,'WECHAT_ACCESS_TOKEN_REFRESH_SKEW_SECONDS');
  return {enabled:true,mode:'server_code_exchange',bindingMode:'crm_exact_match_or_operator_review',appId,appSecret:String(environment.WECHAT_MINIPROGRAM_APP_SECRET),subjectHmacKey:secretKey(environment,'WECHAT_IDENTITY_SUBJECT_HMAC_KEY'),sessionEncryptionKey:secretKey(environment,'MEMBER_SESSION_ENCRYPTION_KEY'),issuer,audience,ttlSeconds,apiTimeoutMs,apiMaxResponseBytes,accessTokenRefreshSkewSeconds,revocationStore:'external_persistent',safeSummary:{enabled:true,provider:'wechat_miniprogram',exchange:'server_only',bindingMode:'crm_exact_match_or_operator_review',autoBindPolicy:'verified_phone_unique_crm_active_in_group_effective_no_risk',loginReturnsProfile:false,groupMembershipApi:false,phoneRequiresExplicitUserAction:true,officialApi:{timeoutMs:apiTimeoutMs,maxResponseBytes:apiMaxResponseBytes,accessTokenCache:'server_memory_only'},session:'opaque_aes_256_gcm',ttlSeconds,persistentRevocationRequired:true,credentialsExposed:false}};
}
