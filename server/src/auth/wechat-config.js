...
function resolveWechatIdentityConfig(environment=process.env){
  ...
  if(environment.MEMBER_BINDING_MODE!=='operator_confirmed_crm')throw new Error('MEMBER_BINDING_MODE 必须为 operator_confirmed_crm；微信登录不能自动确认 CRM 或群成员身份');
  ...
  return {enabled:true,mode:'server_code_exchange',bindingMode:'operator_confirmed_crm',appId,appSecret:String(environment.WECHAT_MINIPROGRAM_APP_SECRET),subjectHmacKey:secretKey(environment,'WECHAT_IDENTITY_SUBJECT_HMAC_KEY'),sessionEncryptionKey:secretKey(environment,'MEMBER_SESSION_ENCRYPTION_KEY'),issuer,audience,ttlSeconds,revocationStore:'external_persistent',safeSummary:{enabled:true,provider:'wechat_miniprogram',exchange:'server_only',bindingMode:'operator_confirmed_crm',loginReturnsProfile:false,groupMembershipApi:false,phoneRequiresExplicitUserAction:true,session:'opaque_aes_256_gcm',ttlSeconds,persistentRevocationRequired:true,credentialsExposed:false}};
}
