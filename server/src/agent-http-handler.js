...
function idempotencyKey(req){const value=String(req.headers?.['idempotency-key']||'');if(!/^[A-Za-z0-9._:-]{8,128}$/.test(value)){const error=new Error('Agent 写操作必须提供有效幂等键');error.code='AGENT_IDEMPOTENCY_REQUIRED';error.statusCode=400;throw error}return value}
...
function createFormalAgentHttpHandler({config,service}={}){
  ...
  const requiredMethods=['listOpportunities','submitDemand','applyToDemand','reviewDemand','prepareDirectionalCandidate','dispatchApplication','recordOwnerDecision','recordOperatorRelay'];
  if(config.enabled&&(!service||requiredMethods.some(method=>typeof service[method]!=='function')))throw new Error('正式 Agent HTTP 路由启用时必须完整注入 004/008 Agent 服务');
  ...
  return async function handleFormalAgentHttp(req,res){
    ...
    try{
      ...
      if(req.method==='POST'&&url.pathname===`${prefix}/demands`){writeJson(res,201,await service.submitDemand({request:req,input:await readJson(req),idempotencyKey:idempotencyKey(req)}));return true}
      match=url.pathname.match(new RegExp(`^${prefix}/demands/([A-Za-z0-9_-]{6,128})/applications$`));
      if(req.method==='POST'&&match){writeJson(res,201,await service.applyToDemand({request:req,demandId:match[1],input:await readJson(req),idempotencyKey:idempotencyKey(req)}));return true}
      ...
      match=url.pathname.match(new RegExp(`^${prefix}/admin/demands/([A-Za-z0-9_-]{6,128})/directional-candidates$`));
      if(req.method==='POST'&&match){const input=await readJson(req);writeJson(res,201,await service.prepareDirectionalCandidate({request:req,demandId:match[1],targetMemberId:input.targetMemberId,criteria:input.criteria,idempotencyKey:idempotencyKey(req)}));return true}
      ...
      match=url.pathname.match(new RegExp(`^${prefix}/applications/([A-Za-z0-9_-]{6,128})/owner-decision$`));
      if(req.method==='PATCH'&&match){const input=await readJson(req);writeJson(res,200,await service.recordOwnerDecision({request:req,applicationId:match[1],decision:input.decision,idempotencyKey:idempotencyKey(req)}));return true}
      match=url.pathname.match(new RegExp(`^${prefix}/admin/applications/([A-Za-z0-9_-]{6,128})/operator-relay$`));
      if(req.method==='PATCH'&&match){const input=await readJson(req);writeJson(res,200,await service.recordOperatorRelay({request:req,applicationId:match[1],decision:input.decision,idempotencyKey:idempotencyKey(req)}));return true}
