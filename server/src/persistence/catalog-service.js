...
const {resolveResourceSection}=require('../resource-sections');
...
class GatewayCatalogReadService{
  ...
  async listResources(identityContext,{limit=50}={}){await this.requireMember(identityContext);try{return (await this.repository.listPublishedResources({limit})).filter(row=>!resolveResourceSection({mobileSection:row.mobile_section,type:row.type,title:row.title,tags:row.tags}).needsClassification).map(safeResource)}catch{throw new CatalogUnavailableError()}}
...
function safeResource(row){const type=String(row.type||''),title=String(row.title||''),tags=Array.isArray(row.tags)?row.tags.map(String).slice(0,10):[];return {id:String(row.id),type,title,summary:String(row.summary||''),tags,accessLevel:String(row.access_level||'active_member'),mobileSection:resolveResourceSection({mobileSection:row.mobile_section,type,title,tags}).section,viewEnabled:true,viewStatus:String(row.preview_status||'preview_not_configured'),downloadEnabled:Boolean(row.download_enabled),publishedAt:row.published_at||null,updatedAt:row.updated_at||null}}
