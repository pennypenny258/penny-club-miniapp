...
const RPC=Object.freeze({begin:'venture_begin_governed_import',stage:'venture_stage_governed_import_rows',stageChunk:'venture_stage_governed_import_chunk',finalize:'venture_finalize_governed_import_batch',review:'venture_review_governed_import_row',rollback:'venture_rollback_governed_import_batch',decision:'venture_record_membership_decision'});
...
class CloudBaseGovernedImportRepository{
  ...
  async stageRows(input){
    const batchId=id(input.batchId,'批次'),rows=Array.isArray(input.rows)?input.rows:[];
    if(rows.length<=500)return this.callRpc(RPC.stage,{p_batch_id:batchId,p_rows:rows});
    for(let offset=0;offset<rows.length;offset+=500)await this.callRpc(RPC.stageChunk,{p_batch_id:batchId,p_rows:rows.slice(offset,offset+500)});
    return this.callRpc(RPC.finalize,{p_batch_id:batchId});
  }
