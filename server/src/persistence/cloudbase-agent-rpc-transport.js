...
const RESULT_STATUSES=Object.freeze({
  [AGENT_OPERATIONS.STAGE_DEMAND]:['pending_review'],
  [AGENT_OPERATIONS.STAGE_APPLICATION]:['submitted'],
  [AGENT_OPERATIONS.REVIEW_DEMAND]:['needs_more_information','rejected','archived','published','private_match_approved'],
  [AGENT_OPERATIONS.UPSERT_DIRECTIONAL_CANDIDATE]:['awaiting_operator_send','duplicate_suppressed'],
  [AGENT_OPERATIONS.DISPATCH_APPLICATION]:['shortlisted','declined'],
  [AGENT_OPERATIONS.RECORD_OWNER_DECISION]:['operator_relay_pending','needs_more_information','declined'],
  [AGENT_OPERATIONS.RECORD_OPERATOR_RELAY]:['relayed','cancelled']
});
...
function hash(value){const text=clean(value,64);if(!/^[a-f0-9]{64}$/.test(text))throw new AgentRpcUnavailableError();return text}
function timestamp(value){if(value===null||value===undefined)return null;const text=clean(value,64),parsed=new Date(text);if(!text||Number.isNaN(parsed.getTime()))throw new AgentRpcUnavailableError();return parsed.toISOString()}
...
class PreparedCloudBaseAgentRpcAdapter{
  ...
  async execute(operation,input){const rpc=RPC_BY_OPERATION[operation];if(!rpc)throw new AgentRpcUnavailableError();const payload=safePayload(operation,input);try{const result=await this.invoker.invoke(rpc,{p_request:payload});if(operation===AGENT_OPERATIONS.LIST_PUBLISHED){if(!Array.isArray(result))throw new AgentRpcUnavailableError();return result.map(row=>({id:id(row.id),type:clean(row.type,40),anonymous_title:clean(row.anonymous_title,120),anonymous_summary:clean(row.anonymous_summary,500),public_tags:tags(row.public_tags),distribution_mode:oneOf(row.distribution_mode,['full_public','redacted_public']),human_review_status:oneOf(row.human_review_status,['approved','approved_with_notes']),status:'published',expires_at:timestamp(row.expires_at)}))}if(!result||typeof result!=='object'||Array.isArray(result))throw new AgentRpcUnavailableError();const safe={id:id(result.id),status:oneOf(result.status,RESULT_STATUSES[operation]),idempotent:result.idempotent===true};if(operation===AGENT_OPERATIONS.UPSERT_DIRECTIONAL_CANDIDATE){safe.suppressedBy14DayWindow=result.suppressed_by_14_day_window===true;if(safe.suppressedBy14DayWindow!==(safe.status==='duplicate_suppressed'))throw new AgentRpcUnavailableError();safe.nextEligibleAt=timestamp(result.next_eligible_at)}return safe}catch(error){if(error instanceof AgentRpcUnavailableError)throw error;throw new AgentRpcUnavailableError()}}
  safeReadiness(){return this.config.safeSummary}
}

module.exports={REQUIRED_BASELINE,CAPABILITY_VERSION,RPC_BY_OPERATION,REQUIRED_RPCS,RESULT_STATUSES,AgentRpcUnavailableError,verifyAgentRpcCapabilityManifest,resolveCloudBaseAgentRpcConfig,safePayload,PreparedCloudBaseAgentRpcAdapter};
