...
const ACCEPTED_MIGRATIONS=new Set([REQUIRED_MIGRATION,'008_admin_session_rbac','014_production_intake_008_baseline']);
...
function resolveGovernedMaterializationConfig(environment=process.env){
  ...
  if(!ACCEPTED_MIGRATIONS.has(environment.CLOUDBASE_PG_MIGRATIONS_APPLIED))throw new Error(`分域物化要求迁移版本至少包含 ${REQUIRED_MIGRATION}`);
