...
test('realistic WeChat shop headers read only four clue categories and silently ignore other columns', () => {
  ...
  const csv = `${headers.join(',')}\n匿名订单号,匿名收件人,${phone},2026-08-01,666,666,已完成,匿名备注,匿名地址,匿名物流号,未退款`;
  const preview = previewPaymentClueCsv(csv,{source:'wechat_shop_order'});
  ...
  for (const forbidden of ['匿名订单号','匿名收件人',phone,'匿名备注','匿名地址','匿名物流号','订单编号','收货地址']) assert.equal(safe.includes(forbidden),false,forbidden);
