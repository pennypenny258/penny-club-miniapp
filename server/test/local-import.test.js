...
test('local import rejects scripts, mismatched signatures and unsafe formula metadata',()=>{
  ...
  assert.throws(()=>validateMetadata({title:'=HYPERLINK("unsafe")',section:'reports_digest'}),error=>error.code==='LOCAL_IMPORT_METADATA_UNSAFE');
});
...
test('rule suggestions are useful without claiming AI or file-text access',()=>{
  const suggestion=generateTagSuggestions({title:'AI 时代软件行业增长报告',filename:'software-growth-report.pdf',extension:'.pdf',mimeType:'application/pdf',section:'reports_digest'});
...
test('images and media require OCR or transcription and sensitive candidates are filtered',()=>{
  const image=generateTagSuggestions({title:'匿名融资图片',filename:'scan.png',extension:'.png',mimeType:'image/png',section:'reports_digest'});assert.equal(image.contentStatus,'ocr_required');assert.match(image.contentNotice,/当前未读取图片内容/);assert.equal(image.aiUsed,false);
