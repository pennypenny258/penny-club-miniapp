...
test('eight formal routes map to the allowlisted Agent contract with member/admin gates first',async()=>{
  ...
  calls.length=0;response=await invoke(handler,'/api/formal-agent/admin/demands/demand_public_1/directional-candidates',{method:'POST',headers:{authorization:'Bearer admin-fixture','idempotency-key':'candidate-http-1'},payload:{targetMemberId:'member_target_1',criteria:{person:'投资人',organization:'产业基金',role:'负责人'}}});assert.equal(response.status,201);assert.equal(response.body.notificationSent,false);assert.deepEqual(calls.slice(0,2).map(call=>call[0]),['admin','repository']);
  calls.length=0;response=await invoke(handler,'/api/formal-agent/applications/application_new_1/owner-decision',{method:'PATCH',headers:{...memberHeaders,'idempotency-key':'owner-http-1'},payload:{decision:'approved_intro'}});assert.equal(response.status,200);assert.equal(response.body.operatorRelayRequired,true);assert.deepEqual(calls.slice(0,2).map(call=>call[0]),['member','repository']);
  calls.length=0;response=await invoke(handler,'/api/formal-agent/admin/applications/application_new_1/operator-relay',{method:'PATCH',headers:{authorization:'Bearer admin-fixture','idempotency-key':'relay-http-1'},payload:{decision:'relayed'}});assert.equal(response.status,200);assert.equal(response.body.deliveryMode,'operator_relay_only');assert.deepEqual(calls.slice(0,2).map(call=>call[0]),['admin','repository']);
});
...
test('formal HTTP boundary rejects invalid methods, JSON, idempotency and oversized bodies safely',async()=>{
  ...
  response=await invoke(handler,'/api/formal-agent/admin/demands/demand_public_1/review',{method:'PATCH',payload:{decision:'approved'}});assert.equal(response.status,400);assert.equal(response.body.code,'AGENT_IDEMPOTENCY_REQUIRED');
...
test('unexpected dependency failures are normalized without leaking upstream details',async()=>{
  const service={listOpportunities:async()=>{throw new Error('raw database credential and row')},submitDemand:async()=>{},applyToDemand:async()=>{},reviewDemand:async()=>{},prepareDirectionalCandidate:async()=>{},dispatchApplication:async()=>{},recordOwnerDecision:async()=>{},recordOperatorRelay:async()=>{}};
